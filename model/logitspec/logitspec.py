...
def logitspec_generate(
    ...
    for _ in range(max_length):
        ...
        draft_ids, tree_attention_mask, tree_position_ids = prepare_attention_inputs(past_len=cur_len, 
        ...
        # t7 = time.time()
        # ? 5. update
        update_ids = torch.cat([draft_ids[:, 0].unsqueeze(0), draft_ids[:, acc_start: acc_end]], dim=-1)
        verify_input_ids = torch.cat([verify_input_ids, update_ids], dim=-1)
        select_kv_idx = torch.cat([torch.arange(cur_len+1), torch.arange(acc_start, acc_end) + cur_len], dim=-1)
        past_key_values = [(k[:, :, select_kv_idx, :], v[:, :, select_kv_idx, :]) for k,v in outputs.past_key_values]
        accept_length_list.append(best_acc_length + 1)       # +1 denotes the next token
        pool.update_pool(update_ids)
        
        if best_acc_length > 0:
            last_logit = logits[:, acc_end-1, :]
        else:
            last_logit = logits[:, 0, :]
        # torch.cuda.synchronize()
        # t8 = time.time()
